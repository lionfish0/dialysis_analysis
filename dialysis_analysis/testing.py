...
class TestDialysisAnalysis(unittest.TestCase):
    def build_test_patient(self,startdate,enddate):
        ...
        lastlab = startdate-datetime.timedelta(10)#arbitrarily earlier? when to start labs?
        datestringlist = []
        labdatestringlist = []
        while date<=enddate:
            gapidx+=1
            if gapidx>=len(gapspacing):
                gapidx = 0
            date+=datetime.timedelta(gapspacing[gapidx])
            datestringlist.append(date.strftime("%d%b%Y"))

            if lastlab-date<datetime.timedelta(-21):
        ...
        for event in range(1+round(len(testdial)/10)):
            ...
            dialnum = dialysis_analysis.strtodatenum(testdial.iloc[row]['dt_date'])
            for i,itrow in testlab.iterrows():
                effect = (dialnum-dialysis_analysis.strtodatenum(itrow['lt_date']))
                if effect<0:
    ...
    def test_no_peeking(self):
        ...
        test_prophets = test_patient.generate_all_prophets(10,['num_date','days_since_dialysis'],['dt_heart_rate_pre'],['lt_value_calcium'])
        ...
        test_prophets_perturbed_last_row = test_patient.generate_all_prophets(10,['num_date','days_since_dialysis'],['dt_heart_rate_pre'],['lt_value_calcium'])
        ...
        assert np.abs(test_prophets[-1].res[0][0,0]-test_prophets_perturbed_last_row[-1].res[0][0,0])<0.01

        assert proph.get_actual()[0]==5 #ensure that the data point was actually changed...
    ...
    def test_population_model(self):
        ...
        test_patients = []
        for it in range(300):
            test_patients.append(self.build_test_patient(datetime.date(2008,7,10),datetime.date(2008,9,10)))

        print("Generating Prophets")
        prophets = []
        for p in test_patients:
            patient_prophets = p.generate_all_prophets(dialysis_analysis.ProphetSimple,300,['num_date','days_since_dialysis'],
        ...
        new_test_patients = []
        for it in range(100):
            new_test_patients.append(self.build_test_patient(datetime.date(2008,7,10),datetime.date(2008,9,10)))

        print("Generating New prophets (no pop prior)")
        prophets = []
        for p in new_test_patients:
            patient_prophets = p.generate_all_prophets(dialysis_analysis.ProphetCoregionalised,300,['num_date','days_since_dialysis'],
                                           ['dt_heart_rate_pre'],['lt_value_albumin'],
                                           delta_dialysis=[('dt_heart_rate_pre','grad',3),
                                                           ('dt_duration','abs')],
        ...
        try:
            test_patients[0].generate_all_prophets(dialysis_analysis.ProphetCoregionalised,300,['num_date','days_since_dialysis'],
                                       ['dt_heart_rate_pre'],['lt_value_albumin'],
                                       delta_dialysis=[('dt_heart_rate_pre','grad',3),
                                                       ('dt_duration','abs')],
